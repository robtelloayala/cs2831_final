...
unet_model_pretrained = fcn_batch(
...
callbacks = [
    ModelCheckpoint(
        filepath='fcn_model_voc_less_augmentation/model_batch_fcn_no_weights.keras', 
        monitor='val_loss',
        save_best_only=True
    ),
    ReduceLROnPlateau(
...
plot_and_save_history(
    history, 
    csv_filename='fcn_model_voc_less_augmentation/training_history_batch_fcn_no_weights.csv', 
    fig_filename='fcn_model_voc_less_augmentation/training_history_batch_fcn_no_weights.png', 
    title='FCN with Batch Normalization'
)