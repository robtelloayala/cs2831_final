...
unet_model_pretrained = unet_model_with_pretrained(
...
def masked_weighted_dice_loss_multiclass(y_true, y_pred, class_weights, ignore_class=255, smooth=1e-6):
    ...
    # Build weights_vec once outside the graph
...
callbacks = [
    ModelCheckpoint(
        filepath='unet_models_voc_less_augmentation/model_unet_pretrained_no_weights.keras', 
        monitor='val_loss',
        save_best_only=True
    ),
    ReduceLROnPlateau(
...
plot_and_save_history(
    history, 
    csv_filename='unet_models_voc_less_augmentation/training_history_pretrained_unet_no_weights.csv', 
    fig_filename='unet_models_voc_less_augmentation/training_history_pretrained_unet_no_weights.png', 
    title='UNet with Pretrained Encoder'
)
