#%%
from unet_models import unet_model_norm
...
def add_sample_weights(image, mask, class_weights, ignore_class=255):
    ...
    mask = tf.cast(mask, tf.int32)
    mask_squeezed = tf.squeeze(mask, axis=-1)  # (batch, h, w)

    # Create a version that replaces 255 with 0
    # so tf.gather doesn't go out of range
...
unet_model_pretrained = unet_model_norm(
...
callbacks = [
    ModelCheckpoint(
        filepath='unet_models_voc_less_augmentation/model_unet_norm_no_weights.keras', 
        monitor='val_loss',
        save_best_only=True
    ),
    ReduceLROnPlateau(
...
plot_and_save_history(
    history, 
    csv_filename='unet_models_voc_less_augmentation/training_history_norm_unet_no_weights.csv', 
    fig_filename='unet_models_voc_less_augmentation/training_history_norm_unet_no_weights.png', 
    title='UNet with Layer Normalization'
)
