...
unet_model_pretrained = unet_model(
...
callbacks = [
    ModelCheckpoint(
        filepath='unet_models_voc_less_augmentation/model_unet_no_weights.keras', 
        monitor='val_loss',
        save_best_only=True
    ),
    ReduceLROnPlateau(
...
plot_and_save_history(
    history, 
    csv_filename='unet_models_voc_less_augmentation/training_history_unet_no_weights.csv', 
    fig_filename='unet_models_voc_less_augmentation/training_history_unet_no_weights.png', 
    title='UNet without normalization'
)
